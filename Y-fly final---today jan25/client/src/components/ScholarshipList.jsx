  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      setScholarships(prev => 
        prev.filter(scholarship => !selectedScholarships.includes(scholarship._id))
      );
      ...
      toast.success(
        `Deleted ${selectedScholarships.length} ${selectedScholarships.length > 1 ? 'scholarships' : 'scholarship'} successfully`
      );
    } catch (error) {
      ...
      <div>
        <div className="flex justify-between mb-5">
          ...
          <div className="flex flex-col items-end">
            ...
            <button
              onClick={() => selectedScholarships.length > 0 ? toggleModal() : toast.warning('No scholarships selected')}
      ...
      <div className="mt-8 text-[0.9rem]">
        <div>
          <div className="font-urban flex bg-[#F9F9F9] py-3 px-3 text-[#30589F]">
            <div className="w-[10%] flex items-center">
              <input
                type="checkbox"
                checked={selectedScholarships.length > 0 && 
        ...
        <div className="flex justify-between items-center mt-5 font-urban">
          <span className="text-sm">
            ...
            {Math.min(currentPage * scholarshipsPerPage, sortedScholarships.length)} of{' '}
            {sortedScholarships.length} entries
          </span>
          <div className="flex space-x-2">
            <button
              className={`px-4 py-2 rounded border ${currentPage === 1 ? 'bg-white cursor-not-allowed text-gray-400 border-gray-300' : 'bg-[#30589F] text-white border-[#30589F]'}`}
            ...
            <button
              className={`px-4 py-2 rounded border ${currentPage === totalPages ? 'bg-white cursor-not-allowed text-gray-400 border-gray-300' : 'bg-[#30589F] text-white border-[#30589F]'}`}
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white px-8 py-6 rounded-lg shadow-lg w-96">
            <h3 className="font-bold text-lg mb-4 text-gray-800">
              Delete {selectedScholarships.length} selected {selectedScholarships.length > 1 ? 'scholarships' : 'scholarship'}?
            </h3>
            ...
            <div className="flex justify-end space-x-4">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-md text-white ${
                  isDeleting ? 'bg-[#f71f1f]' : 'bg-[#a32b2b] hover:bg-[#e74646ef]'
                }`}
                disabled={isDeleting}
              >
                {isDeleting ? (
