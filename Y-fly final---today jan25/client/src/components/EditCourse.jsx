...
const EditCourse = (props, setEditingCourse) => {
    ...
    useEffect(() => {
        const getCourseData = async () => {
            try {
                const response = await axios.get(`http://localhost:5000/courses/get/${props.id}`);
                ...
                setEditCourse({
                    ...
                    testRequirements: data.testRequirements || [{ testRequirementName: "Requirement 1", overallScore: "" }],
                    eligibilityRequirements: data.eligibilityRequirements || [{ requirementType: "", gpaRange: "", backlogRange: "", workExperience: "", entranceExam: "" }],
                    application_requirements: data.application_requirements || [{ requirement: "", isRequired: false }],
                    job_roles: data.job_roles || [{ jobrole: "" }],
                    top_recruiters: data.top_recruiters || [{ recruiters_name: "", logo: null }],
                    scholarship_applicable: data.scholarship_applicable || [{ scholarship: "" }],
    ...
    const handleRemoveJobRole = (index) => {
        const updatedJobRoles = editCourse.job_roles.filter(
            (_, i) => i !== index
        );
        setEditCourse((prevData) => ({
            ...prevData,
                        ...
                        <div className="font-urban mr-10 max-xl:mr-0">
                            <div className="flex text-[0.9rem]">
                                ...
                                <input
                                    ...
                                    value={editCourse?.university_ranking || ""}
