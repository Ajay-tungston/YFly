  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        setFileErr("Invalid file type. Please upload an image (JPG, PNG).");
        return;
      }

      if (file.size > maxSize) {
        setFileErr("File size exceeds 2MB. Please upload a smaller image.");
        return;
      }
      setFileErr("")
      setFormData({ ...formData, university_logo: file });
    }
  };
        ...
        <div className="flex justify-between items-center font-urban">
          <div>
            ...
            <div className="text-[#898C9A] text-[0.8rem]">
              Upload University details here.
            </div>
          </div>
          ...
          <div className="flex items-center text-[0.9rem]">
            ...
            <div className="w-[30%]">
              <input
                ...
                className="w-full bg-[#F9F9F9] border border-[#898C9A] rounded-md placeholder:text-[0.9rem]"
              />
              {fileErr && (
                <p className="text-sm text-[#c52d2d] mt-1 text-right">{fileErr}</p>
              )}
            </div>
          </div>
        </div>
